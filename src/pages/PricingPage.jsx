...
// Map pricing feature text to specific feature slugs
const featureSlugMap = {
  "Dashboard & user management": "dashboard",
  "Clients, teams & projects": "project-management",
  "Manual timesheets & leave": "manual-tracking",
  "Shifts, holidays & meetings": "shift-management",
  "Announcements & company profile": "communication-announcement",
  "Auto Time Tracker": "automatic-tracking",
  "Reports & analytics": "reporting-analytics",
  "Docs portal": "document-management",
  "Manual invoicing": "invoicing",
  "Billing management": "billing-management",
  "Automatic invoicing": "invoicing",
  "Full reporting suite": "reporting-analytics",
  "Recruitment + referrals + docs": "recruitment-management",
};
...
const PricingPage = () => {
  ...
  const getFeatureLink = (featureText) => {
    const slug = featureSlugMap[featureText];
      ...
      >
        ...
        <div className="container relative z-10">
          ...
          <div className="flex justify-center mb-8">
            <div className="bg-white/10 p-1 rounded-xl inline-flex relative backdrop-blur-sm border border-white/20">
              <button
                onClick={() => setBillingInterval('monthly')}
                className={`relative z-10 px-6 py-2 rounded-lg text-sm font-semibold transition-colors duration-300 ${billingInterval === 'monthly' ? 'text-slate-900 bg-white shadow-sm' : 'text-slate-300 hover:text-white'}`}
              >
                Monthly
              </button>
              <button
                onClick={() => setBillingInterval('annual')}
                className={`relative z-10 px-8 py-2 rounded-lg text-sm font-semibold transition-colors duration-300 ${billingInterval === 'annual' ? 'text-slate-900 bg-white shadow-sm' : 'text-slate-300 hover:text-white'}`}
              >
                Annual
                <span className="absolute -top-6 -right-12 z-20 w-16 h-16 flex items-center justify-center text-[10px] leading-[1.1] text-white font-black uppercase text-center bg-gradient-to-br from-red-500 to-rose-600 rounded-full shadow-xl transform rotate-12 border-2 border-dashed border-white/90 pointer-events-none">
                  <span>Save<br/>up to<br/>20%</span>
                </span>
              </button>
            </div>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <ul className="space-y-2 mb-4 flex-1">
                      {(isExpanded ? features.included : features.included.slice(0, 4)).map((feature, i) => {
                        const featureLink = getFeatureLink(feature);
                        const isNonFeatureText = ["Up to", "Flexible seats", "Everything in"].some(prefix => feature.startsWith(prefix));
                        ...
                        return (
                          ...
                          >
                            <span className="flex-shrink-0 w-4 h-4 border border-primary rounded-full flex items-center justify-center text-primary text-xs font-bold mt-0.5">
                              ✓
                            </span>
                            {isNonFeatureText ? (
                              <span>{feature}</span>
                      ...
                      {!isExpanded && features.included.length > 4 && (
                        <li 
                          className="text-sm text-primary font-semibold pl-6 cursor-pointer hover:underline"
      ...
      <section className="py-20 bg-slate-50 border-y border-slate-200">
        <div className="container max-w-5xl mx-auto relative z-10">
          <ScrollReveal className="text-center mb-12">
            <h2 className="font-poppins text-3xl font-bold text-primary-dark mb-4">Detailed Feature Comparison</h2>
            <p className="text-slate-500">See exactly what's included in each plan to find the perfect fit for your team.</p>
          </ScrollReveal>
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-200">
            <div className="overflow-x-auto scrollbar-thin">
              <table className="w-full text-left border-collapse min-w-[700px]">
                <thead>
                  <tr>
                    <th className="p-5 border-b border-slate-200 bg-slate-50/80 sticky left-0 z-20 font-bold text-slate-700 w-1/3 backdrop-blur-sm shadow-[1px_0_0_0_#e2e8f0] align-bottom pb-4">Features</th>
                    {plans.map(plan => (
                      <th key={plan.id} className="p-4 border-b border-slate-200 bg-slate-50/80 text-center font-bold text-primary-dark w-2/9 backdrop-blur-sm">
                ...
                <tbody>
                  {PRICING_DATA.comparisonFeatures.filter(c => c.category !== "Pricing").map((category, catIdx) => (
                    <React.Fragment key={catIdx}>
                      <tr>
                        <td colSpan={plans.length + 1} className="py-3 px-5 bg-slate-100/50 font-semibold text-primary text-sm border-b border-slate-200">
                          {category.category}
                        </td>
                      </tr>
                      {category.features.map((feature, fIdx) => (
                        <tr key={fIdx} className="hover:bg-slate-50/50 transition-colors group">
                          <td className="py-4 px-5 border-b border-slate-100 text-sm text-slate-600 sticky left-0 bg-white group-hover:bg-slate-50/50 z-10 shadow-[1px_0_0_0_#f1f5f9]">
                            {feature.name}
                          </td>
                          {plans.map(plan => {
                            const value = feature.values[plan.id];
                            return (
                              <td key={plan.id} className="py-4 px-5 border-b border-slate-100 text-center text-slate-700">
                                {value === true ? (
